...
def prepare_federated_data(
    ...
    config = DataConfig.for_task("sepsis", data_path)
    X_pool, y_pool = None, None

    if data_path:
        # First try standardized format (data.npz / data.csv)
        try:
            X_pool, y_pool, meta = load_dataset(config)
            result = validate_tabular(X_pool, y_pool, config)
            if not result.is_valid:
                raise ValueError(f"Data validation failed: {result.errors}")
            logger.info(f"[Load] Loaded real data from {data_path}: {len(X_pool)} samples")
            ...
            if local_mode:
                ...
                splits = partition_local(X_pool, y_pool, val_ratio, test_ratio, seed)
                loaders = {0: {
                    "train": DataLoader(SepsisDataset(*splits["train"]), batch_size=batch_size, shuffle=True),
                    "val": DataLoader(SepsisDataset(*splits["val"]), batch_size=batch_size, shuffle=False),
                    "test": DataLoader(SepsisDataset(*splits["test"]), batch_size=batch_size, shuffle=False),
                }}
                metadata = {
                    "num_clients": 1,
                    "input_dim": X_pool.shape[-1],
                    "seq_len": X_pool.shape[1] if X_pool.ndim == 3 else 0,
            ...
            logger.info("[Load] Standardized format not found, trying legacy client_*.npz")
            X_pool, y_pool = None, None

        # Legacy: try loading client_*.npz files
        if X_pool is None:
            try:
                ...
                if local_mode:
                    raise FileNotFoundError(
                        ...
                        f"Run 'python ingest.py --task sepsis --input <your_data>' to ingest data."
                    )
    ...
    if X_pool is None:
        ...
        logger.info(f"[Load] No data found, generating {total} synthetic samples")
