...
def _generate_ecg(n, n_leads=12, seq_len=250, seed=42):
    """Synthetic 12-lead ECG with realistic structure.
    Positive class has higher amplitude in leads V1-V3 (simulated arrhythmia).
    """
    rng = np.random.RandomState(seed)
    y = (rng.rand(n) > 0.7).astype(np.float32)  # ~30% positive (arrhythmia)
    X = rng.randn(n, seq_len, n_leads).astype(np.float32)
    # Add signal: positive class gets amplitude boost in leads 0-2
    pos_mask = y == 1
    X[pos_mask, :, :3] += 0.5
    return X, y
