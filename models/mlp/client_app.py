...
from tasks.fraud.data import prepare_federated_data
...
def _load(pid, n_clients, ptype, alpha):
    ...
    if _data_cache["key"] == key and _data_cache["loaders"]:
        return _data_cache["loaders"][pid]["train"], _data_cache["loaders"][pid]["val"]
...
class MLPClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, params):
        ...
        self.model.load_state_dict({k: torch.tensor(np.array(v)) for k, v in sd.items()})
