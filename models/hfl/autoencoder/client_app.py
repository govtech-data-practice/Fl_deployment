"""Autoencoder ClientApp — anomaly detection via reconstruction error."""
import sys, os
...
from tasks.hfl.anomaly.data import prepare_federated_data

logger = logging.getLogger("autoencoder.client")
...
class AutoencoderClient(fl.client.NumPyClient):
    def __init__(self, pid, n_clients, input_dim=40):
    ...
    def evaluate(self, parameters, config):
        ...
        with torch.no_grad():
            for X, y in val_dl:
                ...
                recon_err = crit(recon, X).mean(dim=-1)
                total_loss += recon_err.mean().item()
                all_scores.append(recon_err.cpu().numpy())
                all_labels.append(y.cpu().numpy())
        all_scores = np.concatenate(all_scores) if all_scores else np.array([])
        all_labels = np.concatenate(all_labels) if all_labels else np.array([])
        # AUC: higher reconstruction error -> anomaly (label=1)
        if len(np.unique(all_labels)) > 1:
            auc = roc_auc_score(all_labels, all_scores)
        else:
            auc = 0.0
        n_samples = len(all_labels)
        return total_loss / max(len(val_dl), 1), n_samples, {"auc": auc}
...
def client_fn(context: Context) -> Client:
    ...
    return AutoencoderClient(pid, n).to_client()
