...
TASK_MODULES = {"sepsis": "tasks.hfl.sepsis.data", "ecg": "tasks.hfl.ecg.data"}
...
def _load(pid, n_clients, ptype, alpha, data_path, max_samples, task):
    ...
    mod = importlib.import_module(TASK_MODULES.get(task, "tasks.hfl.sepsis.data"))
...
class BiLSTMClient(fl.client.NumPyClient):
    ...
    def set_parameters(self, params):
        ...
        # Clamp weights to prevent NaN/Inf from DP noise corrupting LSTM hidden states
    ...
    def _get_device(self, strategy):
        """Use CPU for DP strategies — LSTM on GPU triggers CUDA assert with DP-corrupted weights."""
        if strategy == "dp":
    ...
    def fit(self, parameters, config):
        ...
        # Use CPU for DP strategies to avoid CUDA assert on corrupted LSTM weights
        device = self._get_device(strategy)
    ...
    def evaluate(self, parameters, config):
        ...
        with torch.no_grad():
            for X, y in val:
                X, y = X.to(device), y.to(device); pred = self.model(X).clamp(1e-7, 1-1e-7)
                loss += crit(pred, y).item(); correct += ((pred>0.5).float()==y).sum().item(); total += y.size(0)
        # Move back to GPU for next strategy
        self.model.to(DEVICE)
